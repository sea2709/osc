...
class MJOB_Requests_Front{
	...
	function init_hooks(){
		...
		//add_filter('the_content',array($this, 'mje_set_my_request_content') );
	...
	function mje_request_scripts(){
		...
		wp_enqueue_script( 'mje-requests', MJOBREQUEST_PLUGIN_URL . '/assets/mje-recruit.js', array(), MJOBREQUEST_VERSION, true );
		$translation_array = array(
			'archive_confirm' => __( 'Archive this post?', 'enginethemes' ),
		);
		wp_localize_script( 'mje-requests', 'mje_request', $translation_array );

		if( is_singular(MJOB_RECRUIT) ){
			wp_enqueue_script( 'singular-orders', MJOBREQUEST_PLUGIN_URL . '/assets/custom-order.js', array(), MJOBREQUEST_VERSION, true );
		}
		wp_enqueue_style( 'mje-request-css', MJOBREQUEST_PLUGIN_URL.'/assets/mje_request_style.css',  MJOBREQUEST_VERSION, 'all' );
	...
	function my_recruitments_shortcode(){
		...
		<div class="information-items-detail box-shadow my-recruits">
			...
			<a class="btn-submit-request " href="<?php echo et_get_page_link('post-recruit');?>"><?php _e('Recruit now','enginethems');?><div class="plus-circle"><i class="fa fa-plus"></i></div></a>
            ...
            <?php if ($request_job->have_posts()): ?>
    			...
    			<br />
                	<p class="float-center">
                		<?php printf( __('You don\'t have any recruitment. <a href= "%s">Recruit now</a> now.','enginethemes') ,get_my_recruit_page_link() );?>
	...
	function mjob_recruiments_shortcode( $atts ){
		...
		$mjob_title = __('Latest Recruitments', 'enginethemes');
		ob_start();		?>
            ...
            <div class="container ">
                ...
                <?php else: ?>
                	<p class="float-center">
                		<?php _e('There\'s no recruitment available at the moment. Recruit now.','enginethemes');?>
                		<a class="" href="<?php echo et_get_page_link('post-recruit');?>"><?php _E('Recruit now','enginethemes');?></a>
                	</p>
                <?php endif;?>
	            ...
	            <div class="view-all-jobs-wrap">
	                <a class="btn-order waves-effect waves-light btn-submit mjob-order-action" href="<?php echo get_post_type_archive_link(MJE_REQUEST); ?>">
	                    <?php _e('View all Recruitments', 'enginethemes');?>
	                </a>
	                <a class="btn-submit-request float-right btn-request-abs" href="<?php echo et_get_page_link('post-recruit');?>"><?php _e('Recruit now','enginethems');?><div class="plus-circle"><i class="fa fa-plus"></i></div></a>
	            </div>
	...
	function mje_add_my_request_nav_link(){ ?>
		<li><a href="<?php echo get_my_recruit_page_link(); ?>"><?php _e('My Recruitments', 'enginethemes');?></a></li>
		<?php
	}
	function mje_add_my_request_profile_link(){ ?>
			 <li class="hvr-wobble-horizontal"><a href="<?php echo get_my_recruit_page_link(); ?>"><?php _e('My Recruitments', 'enginethemes'); ?></a></li>
		<?php
	}
	...
	function mjob_request_template_home($post){ ?>
		 <li class="col-lg-12 col-md-12 col-sm-12 col-mobile-12">
			...
			<div class="col-md-2 col-request-link">
				<a class="view-detail" href="<?php echo get_the_permalink($post);?>"><?php _e('View Detail','enginethemes');?></a>
			</div>
		</li>
		<?php
	}
...
function my_request_heading_columns(){?>
	<thead>
        <tr>
            <th><?php _e('Title','enginethemes');?> </th>
            <th class="">
                <?php _e('Date','enginethemes');?>
            </th>
            <th class="td-w-20">
                <?php _e('Budget','enginethemes');?>
            </th>
            <th><?php _e('Status','enginethemes');?></th>
            <th class="text-center"><?php _e('Action','enginethemes');?></th>
        </tr>
    </thead>
<?php }
...
<ul class="request-list list-requests list-mjobs">
	<?php $post_data = array();
	    if( have_posts() ) {
	        ...
	        ?>
	        <div class="not-found"><?php _e('There are no Requests found!', 'enginethemes'); ?></div>
	        <?php
	    }
	?>
	...
	function mje_request_loop_template($convert){ ?>
		 	<div class="full  ">
		 		...
		 		<div class="full row-request-info">
		 			<span><?php printf(__('Posted %s','enginethemes'), date( get_option('date_format'),  strtotime($convert->post_date) ) ); ?></span> |
		 			<span><?php printf(__('%f offers','enginethemes'), $convert->number_offers) ;?> </span>|
			...
			<div class="mjob-single-content list-offers">
				...
				<!-- LIST offers HERE !-->
					<div class="offer-heading">
					    <label class="offer-label"><?php printf(__('%s OFFER(s)','enginethemes'),$query->found_posts);?></label>
					</div>
					<?php
					...
					if( $query->have_posts() ){
					    while( $query->have_posts() ) {
					    	...
					    	_e('There is no offers.','enginethemes');
					}
					?>
...
function mje_btn_submit_request($blank = 0){
	...
	?>
 <a class="btn-submit-request float-right btn-request-abs" <?php echo $target;?> href="<?php echo et_get_page_link('post-recruit');?>"><?php _e('Recruit now','enginethems');?><div class="plus-circle"><i class="fa fa-plus"></i></div></a>
 <?php }
	 ...
	 <div class="banner">
        ...
        <div class="container">
            <div class="search-slider float-center job-items-title">
                ...
                <a href="<?php echo $post_link; ?>" class="btn-post hvr-sweep-to-left waves-effect waves-light"><p class="name-button-post"><?php _e('Recruit now', 'enginethemes'); ?></p> <span class="cirlce-plus"><i class="fa fa-plus"></i></span></a>
            </div>
        </div>
        <div class="header-images">
            ...
            if(!empty($img_url)) {
                ...
                ?>
                <img src="<?php echo $img_url; ?>" alt="<?php _e('Recruit banner', 'enginethemes'); ?>">
                <?php
...
function archive_recruits_sort(){ ?>
	<div class="row functions-items">
	    <div class="col-lg-6 col-md-6 col-sm-6 col-sx-12 no-padding">
	        <h2><?php _e('All Recruitments', 'enginethemes'); ?></h2>
	    </div>
    ...
    <form class="post-job  post et-form" id="">
        <div class="form-group clearfix">
            <div class="input-group">
                <label for="post_title" class="input-label"><?php _e('Recruitment name', 'enginethemes');?></label>
...
?>
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 clearfix">
                    <div class="input-group">
                        ...
                        <label for="et_budget"><?php printf(__('Your budget (%s)', 'enginethemes'), $currency_code);?></label>
