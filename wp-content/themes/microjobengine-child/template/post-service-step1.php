...
<ul class="post-job step-wrapper step-plan" id="step1">
    ...
    if( empty($packs) ){
        ...
        if( is_super_admin() ){
            ...
            echo sprintf(__('Currently, this function is not available. You should setup the Job package <a href="%s">here</a>', 'enginethemes'), $url);
        }
        else{
            echo sprintf(__('Currently, you cannot post a job. For more details, please contact %s', 'enginethemes'), $admin_email);
        }
        return;
    }
    ...
    if($number_of_post >= 1 ) {
        ...
        if( isset($package_data[$sku] ) && $package_data[$sku]['qty'] > 0 ) {
            ...
            if($number_of_post > 1 ) {
                $text = sprintf(__("You can submit %d posts using this plan.", 'enginethemes') , $number_of_post);
            }
            else  {
                $text = sprintf(__("You can submit %d post using this plan.", 'enginethemes') , $number_of_post);
            }
        }else {
            /**
    ...
    <li <?php echo $class_select; ?> data-sku="<?php echo $package->sku ?>" data-id="<?php echo $package->ID ?>" data-price="<?php echo $package->et_price; ?>"
        <?php if( $package->et_price ) { ?>
            data-label="<?php printf(__("You have selected: %s", 'enginethemes') , $package->post_title ); ?>"
        <?php } else { ?>
            data-label="<?php _e("You are currently using the 'Free' plan", 'enginethemes'); ?>"
        <?php } ?>>
        <a class="select-plan" >
