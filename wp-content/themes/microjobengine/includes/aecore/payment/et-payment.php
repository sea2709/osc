...
abstract  class ET_Payment {
	...
	public static function get_support_payment_gateway () {
		...
		$support =	array (
			...
			'cash'		=>	array ('label' => __('Cash', 'enginethemes'),  'active' => -1 ,
	...
	public static function get_currency_list () {
		 ...
		 */
		$currency	=	array (
			'AUD'	=> array (  'code'	=>	'AUD',	'alt'	=>	__("Australian Dollar",'enginethemes'),
								'label'	=>	'AUD',	'icon'	=> 	 'AUD',		'align'	=>	 'left'
							 ),
			'CAD'	=> array (	'code'	=>	'CAD',	'alt'	=>	__("Canadian Dollar",'enginethemes'),
								'label'	=>	'CAD',	'icon'	=> 	 'CAD',		'align'	=>	 'left'
							 ),
			'EUR'	=> array (	'code'	=>	'EUR',	'alt'	=>	__("Euro",'enginethemes'),
								'label'	=>	'EURO',	'icon'	=> 	 '&euro;',	'align'	=>	 'left' 
							 ),
			'USD'	=> array (	'code'	=>	'USD',	'alt'	=>	__("American Dollar",'enginethemes'),
						 		'label'	=>	'USD',	'icon'	=>	'$', 		'align'	=>	'left'	
						 	  ),
			'JPY'	=> array (	'code'	=>	'JPY',	'alt'	=>	__("Japanese Yen",'enginethemes'),
						 		'label'	=>	'JPY',	'icon'	=>	'&yen;',	'align'	=>	'left'	
						 	 ),
			'GBP'	=> array (	'code'	=>	'GBP',	'alt'	=>	__("British Pound",'enginethemes'),
						 		'label'	=>	'GBP',	'icon'	=>	'&pound;',	'align'	=>	'left'	
						 	 ),
			'NZD'	=> array (	'code'	=>	'NZD',	'alt'	=>	__("New Zealand Dollar",'enginethemes'),
						 		'label'	=>	'NZD',	'icon'	=>	'NZD',	'align'	=>	'left'	
						 	 ),
			'CHF'	=> array (	'code'	=>	'CHF',	'alt'	=>	__("Swiss Franc",'enginethemes'),
						 		'label'	=>	'CHF',	'icon'	=>	'CHF',	'align'	=>	'left'	
						 	 ),
			'HKD'	=> array (	'code'	=>	'HKD',	'alt'	=>	__("Hong Kong Dollar",'enginethemes'),
						 		'label'	=>	'HKD',	'icon'	=>	'HKD',	'align'	=>	'left'	
						 	 ),
			'SGD'	=> array (	'code'	=>	'SGD',	'alt'	=>	__("Singapore Dollar",'enginethemes'),
	...
	*/
	...
	function set_checkout ( $nvpstr, $payment_type ) {
		...
		if($payment_type == 'SIMPLEPAYPAL') {
            ...
            ae_paypal_log( 'PayPal Request Args', $paypal );
    ...
    public function successful_request( $posted ) {
	    ae_paypal_log("Check IPN response valid");
        ...
        if ( ! empty( $posted['invoice'] )) {
	        ...
	        $api = self::get_api();
            $invoicePrefix = trim($api['invoice_prefix']);
            if ($invoicePrefix != '') {
                $orderId = substr($posted['invoice'], strlen($invoicePrefix));
            } else {
                $orderId = $posted['invoice'];
            }

            $order_pay = new AE_Order($orderId);
	...
	public function verify_amount($order, $amount) {
		if (number_format($order['total'], 2, '.', '') != number_format($amount, 2, '.', '')) {
			...
			ae_paypal_log("Exit verify amount");
			exit;
		}
	}
