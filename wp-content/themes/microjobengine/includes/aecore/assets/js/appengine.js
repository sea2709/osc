    ...
    */
    AE.Views.File_Uploader = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            var uploadProcessEl = this.processEl = $("#" + this.uploaderID + '_progress');
            ...
            if (this.isS3Upload) {
                ...
                this.config = {
                    ...
                    multipart_params: {
                        'key': 'attachments/' + OSC_S3.shortdate + '/' + time + '-${filename}',
                        'acl': 'public-read',
                        'x-amz-algorithm': 'AWS4-HMAC-SHA256',
                        'x-amz-date': OSC_S3.date,
                        'x-amz-credential': OSC_S3.credential,
                        'policy': OSC_S3.policy,
                        'x-amz-signature': OSC_S3.signature,
                        'x-amz-storage-class': OSC_S3.storageClass
                    },
                    onProgress: function(up, file) {
                        if(uploadProcessEl)
                        {
                            uploadProcessEl.text(file.percent + "%");
                        }
                    },
        ...
        onFileUploaded: function(up, file, res) {
            if (this.isS3Upload) {
                if (res.status == 204) {
                    res.data = {};
                    res.data.url = this.s3URL + file.name;
                    res.data.name = file.name;
                    this.controller.settings.cbS3Uploaded(up, file, res);
    ...
    Views.Carousel = Backbone.View.extend({
        ...
        setupView: function() {
            ...
            this.attachments = [];
            this.attachments = this.model.get('attachments') || [];
            ...
            if (typeof this.carousel_uploader === 'undefined') this.carousel_uploader = new Views.File_Uploader({
                ...
                cbS3Uploaded: function(up, file, res) {
                    if (res.status == 204) {
                        if( $carousel.find('.'+that.options.uploaderID+'-image-list').length > 0 ){
                            var $ul = $carousel.find('.'+that.options.uploaderID+'-image-list');
                        }
                        ...
                        that.model.set('attachments', that.attachments);
                        that.numberOfFile = that.attachments.length;
