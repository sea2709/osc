	...
	function mje_show_authentication_link() {
		...
		if (!is_page_template('page-sign-in.php') && !is_mje_submit_page() && !is_page_template('page-process-payment.php')) {
            ...
            <div class="link-account">
                <ul>
                    <li><a href="" class="<?php echo $sign_in_class; ?>"><?php _e('Sign In', 'enginethemes');?></a></li>
                    <li><span> / </span></li>
                    <li><a href="" class="<?php echo $sign_up_class; ?>"><?php _e('Sign Up', 'enginethemes');?></a></li>
                </ul>
            </div>
            <?php
}
...
if (!function_exists('mje_get_list_attach_files')) {
	function mje_get_list_attach_files($post_id) {
        ...
        $attach = get_post_meta($post_id, 'attachments');
        if (!empty($attach)) {
            foreach ($attach[0] as $k => $v) {
                $str .= "<li><a href='" . $v['url'] . "'><i class='fa fa-paperclip'></i>" . $v['name'] . "</a></li>";
            }
        }
 ...
 * @author Tat Thien
 */
function mje_render_attach_file_button($container_id, $text = "", $isS3Upload = false) {
	$text = $text ? $text : __("Attach file", 'enginethemes');
	ob_start();
	?>
    <div>
        <span class="plupload_buttons" id="<?php echo $container_id ?>_container" <?php if ($isS3Upload):?> data-upload="s3" <?php endif;?>>
    ...
    <div class="custom-progress" id="<?php echo $container_id ?>_progress"></div>
    <?php
$output = ob_get_clean();
	echo $output;
}
